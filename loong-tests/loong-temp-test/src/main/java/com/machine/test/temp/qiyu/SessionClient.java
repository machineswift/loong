...
import com.dtyunxi.thirdparty.qiyu.session.constant.OpenApiTags;
import com.dtyunxi.thirdparty.qiyu.session.model.CommonResult;
import com.dtyunxi.thirdparty.qiyu.session.model.QiyuMessage;
import com.dtyunxi.thirdparty.qiyu.session.util.*;
import com.machine.test.temp.qiyu.model.ApplyStaffInfo;
import com.machine.test.temp.qiyu.model.ApplyStaffResult;
...
public class SessionClient {
    ...
    public QueryQueueResult createTicket(TicketEo eo) throws IOException {
        //JSONObject json = new JSONObject();
        //json.put(OpenApiTags.UID, fromUid);
        String js = JSON.toJSONString(eo);

        JSONObject res = send("openapi/v2/ticket/create", js);
        return JSON.toJavaObject(res, QueryQueueResult.class);
    }

    public QueryQueueResult queryStaffGroup(Map<String, String> map) throws IOException {
        String js = JSON.toJSONString(map);

        JSONObject res = send("openapi/v2/staff/group/list/new", js);
        return JSON.toJavaObject(res, QueryQueueResult.class);
    }

    public QueryQueueResult queryTicketDetail(String ticketId) throws IOException {
        Map<String, String> map = new HashMap<>();
        map.put("ticketId", ticketId);

        JSONObject res = send("openapi/v2/ticket/new/detail", JSON.toJSONString(map));
        return JSON.toJavaObject(res, QueryQueueResult.class);
    }
    ...
    public CommonResult sendImageMessage(String fromUid, String url, String md5, long size, int width, int height) throws IOException {
    ...
    public String uploadFile(String path, String md5) throws IOException {
